...
(rather than the dataset-level ``organelle_classes`` map), because several datasets renumber organelles
per volume (e.g. empiar_12885 mito in {1,2}, ER in {6,8}; empiar_10994 ER in {2,3} + subtypes). Matching
is by name, and each match's integer ``value`` is read per crop; instance-encoded entries (which carry
...
def _matching_entries(organelles_present: dict, organelle: str) -> list[tuple[str, dict]]:
    ...
    target organelle is never silently dropped to background.
    """
...
def canonical_mask(label: np.ndarray, organelles_present: dict, organelle: str) -> np.ndarray:
    ...
    # an instance-encoded target co-occurs with other value-bearing organelles in the same label
    # map, subtract their values so the mask does not absorb every other organelle. (A no-op wherever
    # instance-target crops are single-organelle.)
    if used_instance:
        other_vals = [
