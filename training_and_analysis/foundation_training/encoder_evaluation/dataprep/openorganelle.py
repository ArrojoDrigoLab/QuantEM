...
Each crop has dual-orientation raw tiles (raw_xy / raw_xz) sharing one 3D seg volume at its own
native resolution. raw<->seg alignment is computed in physical nanometres rather than integer voxels
(the rounded voxel bbox is off by +/-1-2 voxels); seg is resampled onto the EM grid by nearest
neighbour, so label ids are never interpolated; and each (orientation, plane) is an independent 2D
sample. seg_er==255 ("unknown") maps to ignore.
"""
...
def iter_oo_samples(crop_dir: str | Path, organelle: str) -> list[OOSample]:
    ...
    em_res = man["original_image"]["resolution_nm_zyx"]  # [z,y,x]
    ...
    for orient in ("raw_xy", "raw_xz"):
        ...
        a0, a1 = int(a0), int(a1)
        c0, c1 = int(c0), int(c1)
        # Densification metadata (optional: a manifest without these fields resolves to the baseline
        # 200 nm spacing and no dense-only planes).
        ...
        for k, plane_nm in enumerate(rw["plane_physical_nm"]):
            ...
            if ai == 0:
                ...
                seg2d = seg_vol[:, :, si]
            # seg2d's footprint is the full (unclamped) annotation window [a0,a1]x[c0,c1]. Resize it to
            # that full grid first, then crop to the clamped sub-window — otherwise it would squash the
            # whole seg slice into the clamped extent and shift/scale the labels when the bbox runs off
            # the tile edge.
