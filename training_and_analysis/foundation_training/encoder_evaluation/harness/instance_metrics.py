...
def postproc_instances(prob: np.ndarray, valid: np.ndarray, threshold: float = 0.5,
    ...
    if min_size > 0 and n > 0:
        ...
        remove = sizes < min_size            # O(n_labels) bool LUT indexed by label id
        remove[0] = False                    # never drop background
        if remove.any():
            lab[remove[lab]] = 0             # O(n_px) gather, not O(n_px * n_small) np.isin
...
def instance_metrics(prob: np.ndarray, gt_inst: np.ndarray, valid: np.ndarray, *,
    ...
    """PQ/SQ/RQ, AP@[.5:.95], VI for one crop. ``gt_inst`` = GT instance ids (0=bg/ignore)."""
    ...
    # per-instance mean prob in one O(n_px) pass: ndi.mean over all ids at once, rather than one
    # full-image boolean pass (prob[pred == pid]) per instance.
    _means = np.atleast_1d(ndi.mean(prob, labels=pred, index=pred_ids)) if pred_ids.size else []
    score = {int(pid): float(m) for pid, m in zip(pred_ids.tolist(), _means)}
