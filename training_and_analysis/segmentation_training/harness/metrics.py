"""Segmentation metrics for the harness.
...
Per-head sets:
  ...
  * mito: the above (minus cldice) + the semantic-CC pq, sq, rq, ap, vi and, for a decoder that
    exposes a native instance post-proc, inst_pq, inst_sq, inst_rq, inst_ap, inst_vi
    (both instance sets are added in evaluate.py, via ``instance_eval.dual_instance_metrics``)

Implementation uses numpy + scipy.ndimage only (no numpy-BLAS matmul, no sklearn). ``clDice`` uses
``skimage.skeletonize`` when available, with a scipy morphological-skeleton fallback so the module
runs without skimage installed.
"""
...
INSTANCE_KEYS = ("pq", "sq", "rq", "ap", "vi",
                 # true-instance metrics (the decoder's own native post-proc: mutex-watershed on affinities,
                 # centre-offset grouping, or per-query-mask assignment), reported alongside the
                 # semantic-CC pq/sq/rq/ap/vi above.
                 "inst_pq", "inst_sq", "inst_rq", "inst_ap", "inst_vi")
ALL_KEYS = SEMANTIC_KEYS + INSTANCE_KEYS
LOWER_BETTER = frozenset({"hd95", "vi", "inst_vi"})  # for these, a lower value is better (worst = max)
...
def _band(mask: np.ndarray, valid: np.ndarray, d: int) -> np.ndarray:
    if not mask.any():
        return np.zeros_like(mask, dtype=bool)
    return mask & ~ndi.binary_erosion(mask | ~valid, iterations=max(d, 1), border_value=0)
...
def per_crop_metrics(pred_bin: np.ndarray, gt_label: np.ndarray, *, prob: np.ndarray | None = None,
    ...
    if gb.sum() == 0 and pb.sum() == 0:
        ...
        bp_prec = float((dt_g[pb] <= theta).mean())
        bp_rec = float((dt_p[gb] <= theta).mean())
        ...
        out["hd95"] = float(max(np.percentile(dt_g[pb], hd95_pct), np.percentile(dt_p[gb], hd95_pct)))
    if out["hd95"] is None:
