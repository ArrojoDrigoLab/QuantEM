; QuantEM vendored NSIS template.
;
; This is the verbatim installer template embedded in @tauri-apps/cli 2.11.4
; (tauri-bundler), vendored so one page can be inserted in the right position
...
Function un.ConfirmShow ; Add add a `Delete app data` check box
  ...
  IntOp $6 $6 / 96
  IntOp $7 $7 / 96
; --- QuantEM insertion (uninstall checkbox names the data\ folder) ----------
; The stock $(deleteAppData) says only "Delete the application data". QuantEM
; keeps everything under <INSTDIR>\data, so the copy (QUANTEM_DELETE_DATA_TEXT,
; defined in nsis/hooks.nsh) names that folder and its contents. The control
; grows from 25 to 40 px for the wrapped text (__NSD_CheckBox_STYLE already
; includes BS_MULTILINE); the confirm dialog has ~100 px free below it.
!ifdef QUANTEM_DELETE_DATA_TEXT
  IntOp $7 40 * $2
  IntOp $7 $7 / 96
  System::Call 'user32::CreateWindowEx(i r3, w "${__NSD_CheckBox_CLASS}", w "${QUANTEM_DELETE_DATA_TEXT}", i ${__NSD_CheckBox_STYLE}, i r4, i r5, i r6, i r7, p r1, i0, i0, i0) i .s'
!else
