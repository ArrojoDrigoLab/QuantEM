...
def run_identity_from_segmenter(
    ...
    *,
    ...
    (``model_spec``, ``fg_threshold``, ``adapter_id``), each of which is
    optional: a segmenter that does not expose one records ``None`` for that
    field rather than guessing. ``pack_id_fallback`` is the resolved source
    model, used when the segmenter has no ``model_spec`` -- the two coincide for
    every released pack.
    """
